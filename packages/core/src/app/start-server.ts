...
export async function startServer(
  ...
  try {
    ...
    // Logical restore coordinated by the control plane after the runtime exists.
    // Concrete providers may also embed restore in bootstrap; calling download here
    // keeps the SaveStorage port exercised and is a no-op when nothing was stored yet.
    if (plan.restoreSave) {
      await deps.saveStorage.download(gameId, adapter.savePaths());
    }
