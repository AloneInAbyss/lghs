...
export async function stopServer(deps: AppDeps): Promise<StopResult> {
  ...
  try {
    ...
    if (gameId !== undefined && runtimeId !== undefined) {
      ...
      const workingDirectory = adapter.bootstrapPlan().workingDirectory;
      const syncContext = { runtimeId, workingDirectory };
      ...
      await deps.saveStorage.upload(gameId, adapter.savePaths(), syncContext);
    }
